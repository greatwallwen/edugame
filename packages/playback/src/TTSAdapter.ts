...
export class TTSAdapter {
  ...
  /**
   * 从预生成 mp3 URL 播放（跳过 TTS API 调用）。
   * manifest 中 action.ttsUrl 非空时由 ActionEngine 调用本方法。
   * 播放完成后调 onDone；失败时 fallback 到 speak(text)。
   */
  speakFromCache(url: string, onDone: () => void, onError?: (e: Error) => void): void {
    if (this.disposed) { onDone(); return; }
    const audio = new Audio(url);
    audio.playbackRate = this.opts.rate;
    ...
    audio.onerror = () => {
      this.currentAudio = null;
      if (onError) onError(new Error(`mp3 加载失败: ${url}`));
      else onDone();
    };
    void audio.play().catch(() => {
      this.currentAudio = null;
      if (onError) onError(new Error(`mp3 播放被阻止: ${url}`));
      else onDone();
    });
  }
