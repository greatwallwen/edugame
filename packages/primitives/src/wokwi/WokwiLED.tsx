...
export interface WokwiLEDProps {
  /** 是否点亮 */
  value?: boolean;
  /** 0..1，默认 1 */
  brightness?: number;
  /** 主体颜色 */
  color?: 'red' | 'green' | 'blue' | 'yellow' | 'orange' | 'white' | 'purple' | string;
  /** 发光颜色（缺省按 color 查 lightColors 表） */
  ...
  /** 容器宽度（CSS），默认 40px */
  width?: number | string;
  /** 容器外层 className（用于上层加 highlight 等） */
  className?: string;
  /** Phase G3 · 让 ActionRunner 能高亮该元素 */
...
export function WokwiLED({
  ...
  const lookup = lightColors[color.toLowerCase()];
  const lightColorActual = lightColor ?? lookup ?? color;
  ...
  return (
    ...
    >
      <svg
        ...
        aria-label={label || `${color} LED`}
        role="img"
      >
        <filter id="dgb-led-light1" x="-0.8" y="-0.8" height="2.2" width="2.8">
          <feGaussianBlur stdDeviation="2" />
        </filter>
        <filter id="dgb-led-light2" x="-0.8" y="-0.8" height="2.2" width="2.8">
        ...
        {lightOn && (
          <g className="dgb-wokwi-led-light">
            <ellipse
              cx="8"
              cy="10"
              rx="10"
              ...
              filter="url(#dgb-led-light2)"
              style={{ opacity }}
            />
            <ellipse cx="8" cy="10" rx="2" ry="2" fill="white" filter="url(#dgb-led-light1)" />
            <ellipse
              cx="8"
              cy="10"
              rx="3"
