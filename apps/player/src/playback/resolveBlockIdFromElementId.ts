...
export function resolveBlockIdFromElementId(
  ...
  // 段落级 text spotlight：dgb-text-{blockId}-s{N} → 提取 blockId
  if (elementId.startsWith('dgb-text-')) {
    const rest = elementId.slice('dgb-text-'.length); // e.g. "p1c-text-s0"
    const lastDash = rest.lastIndexOf('-s');
    if (lastDash > 0) {
  ...
  for (const [prefix, kind] of probes) {
    ...
    for (const b of page.blocks as NodeBearingBlock[]) {
      ...
      // 记录同类首个 block 作为 fallback（animation block 通常只有一个）
      if (kind === 'animation' && !fallbackBlockId) fallbackBlockId = b.id;
      if (Array.isArray(b.nodes) && b.nodes.some((n) => n.id === nodeId)) {
        return b.id;
      }
    }
    // 在 animation block 无 nodes 数据时仍能驱动 spotlight + scrollIntoView
    if (kind === 'animation') return fallbackBlockId;
    return null;
  }
